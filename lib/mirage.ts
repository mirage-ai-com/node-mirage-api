...
class Mirage {
      ...
      .on("response", (response) => {
        ...
        if (response.statusCode >= 400) {
          return reject({
            ...
            message: (
              "Stream closed, got " + response.statusCode +
              " error from server."
            )
          });
        }
  ...
  */
  private getRequestOptions(trace: string = null, stream: boolean = false) {
    ...
    if (stream === true) {
      // Disable any form of compression if streaming \
      //   (we do not want to decode any eg. compressed chunk of data)
      headers["Accept-Encoding"] = "identity";
    }

    if (trace !== null) {
      // Stamp request with provided trace identifier (this is optional \
      //   but can be used to track request across Mirage backend systems \
